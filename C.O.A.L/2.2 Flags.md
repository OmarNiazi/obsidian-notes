...
| **Direction**  |  DF   |    `UP`     | Control | String ops decrement (`DN`) or increment (`UP`). |
| **Interrupt**  |  IF   |    `EI`     | System  | Interrupts Enabled (`EI`) or Disabled (`DI`).    |
| **Sign**       |  SF   |    `PL`     | Status  | Result is negative (`NG`) or positive (`PL`).    |
| **Zero**       |  ZF   |    `ZR`     | Status  | Result is zero (`ZR`) or not zero (`NZ`).        |
| **Aux. Carry** |  AF   |    `AC`     | Status  | BCD carry from bit 3 to 4.                       |
| **Parity**     |  PF   |    `PE`     | Status  | Parity is even (`PE`) or odd (`PO`).             |
...
### 1\. Carry Flag (CF)
  ...
  * **Abbreviation:** `CF`
  * **VS Debugger:** `CY` (Set, 1) / `NC` (Not Clear, 0)
...
1.  **`ADD` / `ADC` (Add / Add with Carry):**
    ...
    .code
    mov al, val1
    add al, 1      ; 255 + 1 = 256. Result is 00h.
...
5.  **`MUL` / `IMUL` (Multiply):**
      ...
      * For `MUL BL` (8-bit), the result is in `AX`. `CF` is set if `AH` is not 0.
...
### 2\. Zero Flag (ZF)
  ...
  * **Abbreviation:** `ZF`
  * **VS Debugger:** `ZR` (Set, 1) / `NZ` (Not Zero, 0)
...
### 3\. Sign Flag (SF)
  ...
  * **Abbreviation:** `SF`
  * **VS Debugger:** `NG` (Negative, 1) / `PL` (Plus, 0)
...
### 4\. Overflow Flag (OF)
  ...
  * **Abbreviation:** `OF`
  * **VS Debugger:** `OV` (Overflow, 1) / `NV` (No Overflow, 0)
...
### 5\. Parity Flag (PF)
  ...
  * **Abbreviation:** `PF`
  * **VS Debugger:** `PE` (Parity Even, 1) / `PO` (Parity Odd, 0)
